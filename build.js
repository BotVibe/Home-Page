...
let footerLinksHtml = '';
if (data.profile.social.github && data.profile.social.github !== '#') {
    footerLinksHtml += `<a href="${data.profile.social.github}" target="_blank" rel="noopener noreferrer" class="neo-btn px-4 py-2 bg-[var(--card-bg)] font-mono text-sm font-bold hover:bg-[var(--accent-yellow)] transition-colors">GITHUB</a>`;
}
if (data.profile.social.linkedin && data.profile.social.linkedin !== '#') {
    footerLinksHtml += `<a href="${data.profile.social.linkedin}" target="_blank" rel="noopener noreferrer" class="neo-btn px-4 py-2 bg-[var(--card-bg)] font-mono text-sm font-bold hover:bg-[var(--accent-yellow)] transition-colors">LINKEDIN</a>`;
}
if (data.profile.social.email && data.profile.social.email.user) {
    footerLinksHtml += `<a href="#" id="obf-email-footer" class="neo-btn px-4 py-2 bg-[var(--card-bg)] font-mono text-sm font-bold hover:bg-[var(--accent-yellow)] transition-colors">EMAIL</a>`;
}
...
data.techStack.tools.forEach(t => {
    toolsHtml += `<div class="flex items-center gap-2"><span class="w-3 h-3 border-2 border-black dark:border-white" style="background-color: ${t.color}"></span><span class="font-bold">${t.name}</span></div>`;
});
...
Object.keys(data.projects).forEach(k => {
    ...
    category.items.forEach(item => {
        ...
        if (item.image && item.image !== "[IMG_PLACEHOLDER]") {
            imageHtml = `<div class="h-40 neo-border mb-4 flex items-center justify-center font-mono text-xs bg-gray-50 dark:bg-gray-900"><img src="${item.image}" alt="${item.title}" class="w-full h-full object-cover" onerror="this.style.display='none'; this.parentNode.innerHTML='[IMG_PLACEHOLDER]'"/></div>`;
        } else {
            imageHtml = `<div class="h-40 neo-border mb-4 flex items-center justify-center font-mono text-xs bg-gray-50 dark:bg-gray-900">[IMG_PLACEHOLDER]</div>`;
        }
        itemsHtml += `
        <a href="${item.link}" target="_blank" class="snap-start min-w-[300px] md:min-w-[400px] neo-card bg-[var(--card-bg)] dark:bg-[var(--card-bg-dark)] p-6 group block">
            ${imageHtml}
    ...
    projectsHtml += `
    <section class="w-full py-12 mb-8" style="border-top: 3px solid ${category.color}; border-bottom: 3px solid ${category.color}; background-color: color-mix(in srgb, ${category.color} 8%, transparent);">
        <div class="max-w-6xl mx-auto px-6">
            <div class="flex items-center gap-3 mb-6">
                <span class="w-4 h-4 border-2 border-black" style="background-color: ${category.color}"></span>
